...
import { SwitchNShootGame } from './SwitchNShootGame';

interface SwitchNShootLauncherProps {
  onBack: () => void;
  onOpenSettings?: () => void;
...
const DIFFICULTY_INFO = [
  {
    ...
    multiplier: 1.0, description: 'Slow enemies, 5 lives. Learn the mechanics.',
    features: ['Slow enemy speed', '5 lives', '1.0x multiplier'],
  },
  {
    id: 'medium', name: 'Medium', icon: Zap,
    color: 'from-yellow-600 to-orange-600', borderColor: 'border-yellow-500',
    multiplier: 1.5, description: 'More enemies, faster. Think quick!',
    features: ['Normal speed', '3 lives', '1.5x multiplier'],
  },
  {
    id: 'hard', name: 'Hard', icon: Flame,
    color: 'from-red-600 to-pink-600', borderColor: 'border-red-500',
    multiplier: 2.0, description: 'Relentless spawns. Only 2 lives.',
    features: ['Fast enemies', '2 lives', '2.0x multiplier'],
  },
];

export function SwitchNShootLauncher({ onBack, onOpenSettings, onNavigateToLeaderboard, onGameComplete }: SwitchNShootLauncherProps) {
  const [showGame, setShowGame] = useState(false);
  const [selectedDifficulty, setSelectedDifficulty] = useState<'easy' | 'medium' | 'hard' | null>(null);

  const getTopScores = () => {
    try {
      const scores = JSON.parse(localStorage.getItem('switchnshoot_scores') || '[]');
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-5xl mx-auto">
          <motion.div className="text-center mb-10" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="text-6xl font-bold mb-4 bg-gradient-to-r from-purple-400 via-red-400 via-cyan-400 to-yellow-400 bg-clip-text text-transparent">
              SWITCH 'N' SHOOT
            </h1>
            <p className="text-lg text-gray-400 max-w-xl mx-auto">
              Match your color to the enemy's color and destroy them before they reach you. Switch fast, shoot faster!
            </p>
          </motion.div>
          ...
          <motion.div className="bg-[#0a0a18] rounded-2xl border border-purple-900/50 p-6 mb-8"
            ...
            <div className="grid grid-cols-4 gap-4 text-sm text-gray-400">
              <div>
                <div className="text-purple-400 font-medium mb-2">🚀 Move</div>
                <p>Use arrow keys or A/D to move your ship left and right across the bottom.</p>
              </div>
              <div>
                <div className="text-red-400 font-medium mb-2">🎨 Switch Color</div>
                <p>Press 1–4 to switch your bullet color to match incoming enemies.</p>
              </div>
              <div>
                <div className="text-cyan-400 font-medium mb-2">💥 Shoot</div>
                <p>Hold SPACE to fire. Only matching colors destroy enemies — wrong colors miss!</p>
              </div>
              <div>
                <div className="text-yellow-400 font-medium mb-2">🔥 Combos</div>
                <p>Destroy enemies in quick succession to build combos and multiply your points.</p>
              </div>
            </div>
          </motion.div>
          ...
          {topScores.length > 0 && (
            <motion.div className="bg-[#0a0a18] rounded-2xl border border-gray-800 p-6"
              ...
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3"><Trophy className="w-5 h-5 text-yellow-500" /><h2 className="text-xl font-bold">Your Best Scores</h2></div>
